...
// src/functions/addEmployee/handler.ts
__export(exports, {
  main: () => main
});
...
var addEmployee = async (event) => {
  ...
  try {
    ...
    let validate2 = await employeeSchema.validateAsync(employee);
    if (validate2.error) {
      throw new schemaValidation(validate2.error, 400);
    }
    await dynamo.put({
      TableName: "employees",
      Item: __spreadProps(__spreadValues({}, event.body), { id: v4() })
    }, function(err) {
      if (err) {
        return formatJSONResponse({
